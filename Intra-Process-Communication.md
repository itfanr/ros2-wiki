...
As you can see in the example image above, we have one image with all of pointers the same and then another image with the same pointers as the first image for the first two entries, but the last pointer on the second image is different. To understand why this is happening consider the graph's topology:
...
This is a very rough first draft. There is a lot of room for improvement, even beyond what I enumerated above. We'll start to improve performance as we dig into the details of the system, build up better understanding of exactly what our middleware vendors are doing, and as we try alternative strategies for implementing intra process.

### What's Missing

Those are some things we can improve with what's already there. But there are also some things we'd like to add on top of this that are pretty interesting and some things that are just necessary.
...
This is the coolest (in my opinion) upcoming feature that we didn't get to in this demo.
...
- An executable for your node.
 - The executable can run the node in it's own process, or serve as a proxy while the node runs in a different container.

We'll also need to develop the container, which can run nodes inside of itself and is controller externally by ROS primitives like Services.
